...
def update_analysis_state(status, detail):
    """분석 상태를 파일에 기록 (Dashboard 공유용)"""
    try:
        state = {
...
def generate_daily_briefing():
    ...
    update_analysis_state("running", "🚀 분석 시작: 데이터 수집 중...")
    ...
    try:
        ...
        update_analysis_state("running", "🏢 Stage 1/3: 부동산 및 자산 데이터 분석 중...")
        ...
        print("   ✅ 자산 분석 완료.")
        time.sleep(2) # API 쿨타임

        # -----------------------------------------------------
        # Stage 2: 기술 & 커리어 분석
        # -----------------------------------------------------
        print("\n[Stage 2/3] 💻 기술 & 채용 트렌드 분석 중...")
        update_analysis_state("running", "💻 Stage 2/3: 기술 트렌드 및 채용 정보 분석 중...")
        ...
        update_analysis_state("running", "⚡ Stage 3/3: 최종 전략 리포트 작성 중...")
        ...
        * (내 사고를 확장할 질문 1개)
        ...
        update_analysis_state("completed", "🎉 모든 분석이 완료되었습니다! 아래 리포트를 확인하세요.")
        print("\n" + "="*50)
        print("🎉 모든 분석 완료! 결과:")
        print("="*50)
        ...
        print(f"❌ {error_msg}")
        update_analysis_state("failed", error_msg)
